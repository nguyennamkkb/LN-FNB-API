...
export class TableService {
  constructor(
    @InjectRepository(TableEntity) private repository: Repository<TableEntity>
  ) {}
  ...
  async findAll(
    ...
    limit: number,
    param: any
  ): Promise<[TableEntity[], number]> {
    let where = {};
    if (param.user_id) {
      where["user_id"] = param.user_id;
    }
    if (param.keySearch) {
      where["name"] = Like("%" + param.keySearch + "%");
    }
    if (param.status) {
  ...
  async updateTableSelected(table: string[]): Promise<any> {
    ...
    const sqlString =
      "update table_entity set status = 2, updateAt=" +
      Date.now() +
      " where name in (" +
      listTable +
      ")";
  ...
  async updateTableDatTruoc(table: string[]): Promise<any> {
    ...
    listTable = listTable.substring(0, listTable.length - 1);
    const sqlString =
      "update table_entity set status = 4, updateAt=" +
      Date.now() +
      " where name in (" +
      listTable +
      ")";
  ...
  async updateTableWStatus(table: string[], status: number): Promise<any> {
    // Đặt lại trạng thái
    let listTable = "";
    for (let index = 0; index < table.length; index++) {
      const element = table[index];
    ...
    listTable = listTable.substring(0, listTable.length - 1);
    const sqlString =
      "update table_entity set status = " +
      status +
      ", updateAt=" +
      Date.now() +
      " where name in (" +
  ...
  async resetTable(table: string[]): Promise<any> {
    ...
    listTable = listTable.substring(0, listTable.length - 1);
    const sqlString =
      "update table_entity set status = 1, updateAt=" +
      Date.now() +
      " where name in (" +
      listTable +
      ")";
